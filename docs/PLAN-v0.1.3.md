...
## 4. A3：画布会话框「+ 文件」通道（v0.1.2 搁置，工作量 2–3h）

**内容**：画布会话框加「+ 文件」按钮，直接传本地文件进 ComfyUI `input/`。

**硬前置**：浏览器 `<input type="file">` 给的是 File 对象、不是绝对路径，而 `attach_file` 收 `path`——需先确认 client → host 传参通道（v0.1.2 方案开放问题 #1）。若通道不可行，降级为「agent 引导用户给路径」。

**注**：A1（host 侧 attach_file）已让 agent 能传文件，A3 是「手动点按钮直传」的锦上添花，可再降级/砍掉。
