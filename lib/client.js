...
function ComfyUISplitToggle(props) {
  ...
  var snapshot = React.useSyncExternalStore(
    function (cb) { return scope && typeof scope.subscribe === "function" ? scope.subscribe(cb) : function () {}; },
    function () { return scope && typeof scope.getSnapshot === "function" ? scope.getSnapshot() : null; },
    function () { return scope && typeof scope.getSnapshot === "function" ? scope.getSnapshot() : null; },
  );
...
function apply(ctx) {
  // 2026-09-24: 核心 #4587 移除了客户端 settingsScope 服务，且 cordis ctx 代理对
// 未注入服务的属性访问（ctx.settingsScope）会直接抛 "cannot get property...
// without inject"，导致客户端 apply 失败。改用 ctx.get()（缺失返回 undefined）。
var settingsScopeService = typeof ctx.get === "function" ? ctx.get("settingsScope") : undefined;
  var scope = settingsScopeService && typeof settingsScopeService.bind === "function"
    ? settingsScopeService.bind({ namespace: NAMESPACE })
    : undefined;
