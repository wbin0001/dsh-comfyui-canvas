...
function setSessionView(sc, sessionId, mode) {
  ...
  if (keys.length > MAX_SESSION_VIEWS) {
    ...
    var drop = keys.length - MAX_SESSION_VIEWS;
    var keep = {};
...
function ComfyUICanvasView(props) {
  ...
  // 桥接鉴权告警（非启动路径，画布激活时按需展示）：只有当 DSH 侧配了
  // token、但桥接握手显示"错误 token 也能通过"时才亮红字——这是真正的
  // 配置错位（ComfyUI 端没设 DSH_BRIDGE_TOKEN）。正常/未配 token 不打扰。
  if (bridgeProbe && bridgeProbe.bridgeAuthEffective === false && tokenSet) {
    hostChildren.push(React.createElement("div", {
      style: {
        position: "absolute", top: "8px", left: "50%", transform: "translateX(-50%)",
        zIndex: 45, maxWidth: "86%",
        background: "rgba(248,81,73,.12)", color: "#f85149",
        border: "1px solid rgba(248,81,73,.4)", borderRadius: "6px",
...
function ComfyUISettingsRow(props) {
  ...
  if (probe.bridgeAuthEffective === true) {
    ...
    probeColor = bridgeToken ? "#f85149" : "#d29922";
  }
