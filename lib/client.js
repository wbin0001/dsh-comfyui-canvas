...
function buildCanvas() {
  ...
  // 重启 / 关闭 ComfyUI 控制按钮（放在状态圆点前面）：点击派发自定义事件，
  // apply 里的监听器把它写进 settings（comfyControl），host watcher 执行
  // kill / restart；结果由画布的可达性探测自动反映到状态圆点。
  var controlButton = function (title, inner, action) {
    var btn = document.createElement("button");
    btn.type = "button";
    btn.title = title;
    btn.setAttribute("aria-label", title);
    btn.innerHTML = inner;
    btn.style.cssText = "background:transparent;border:none;padding:0;color:var(--dsw-alias-state-business-primary,#8ab4f8);cursor:pointer;display:inline-flex;align-items:center;";
    btn.addEventListener("click", function () {
      document.dispatchEvent(new CustomEvent("dsh-comfyui-control", { bubbles: true, detail: { action: action } }));
    });
    return btn;
  };
  var restartBtn = controlButton(
    "重启 ComfyUI",
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 2v6h-6"/><path d="M21 13a9 9 0 1 1-3-7.7L21 8"/></svg>',
    "restart"
  );
  var stopBtn = controlButton(
    "关闭 ComfyUI",
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 2v10"/><path d="M18.4 6.6a9 9 0 1 1-12.77.04"/></svg>',
    "stop"
  );
...
function ComfyUIControl(props) {
  ...
  var cardStyle = {
    ...
    // 内容纵向排列、全部居中：标题/说明/错误/按钮都在卡片中间，按钮与画布左右距离相等。
  ...
  return React.createElement("div", { style: rowStyle },
    ...
    React.createElement("div", {
      style: {
        ...
        borderTop: "1px solid var(--dsw-alias-border-l1,rgba(128,128,128,.25))",
        display: "flex", flexDirection: "column", gap: "8px", alignItems: "flex-start",
      },
    },
      React.createElement("div", { style: { fontSize: "13px", fontWeight: "600", color: "var(--dsw-alias-label-primary,#e8e8e8)" } }, "插件版本与更新"),
      React.createElement("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9a9aa2)", lineHeight: 1.5 } },
        "当前版本 v" + (value.pluginVersion || "?") +
        (upStatus.latestVersion ? " · 最新 v" + upStatus.latestVersion : "")
      ),
      upStatus.status && upStatus.status !== "idle" ? React.createElement("div", {
        style: {
          fontSize: "12px", lineHeight: 1.5, whiteSpace: "pre-wrap",
          color: upStatus.status === "error"
            ? "var(--dsw-alias-state-error-primary,#f85149)"
            : upStatus.status === "available"
              ? "var(--dsw-alias-state-warn-primary,#d29922)"
              : "var(--dsw-alias-label-secondary,#9a9aa2)",
        },
      }, upStatus.message || "") : null,
      upStatus.status === "done" ? React.createElement("div", {
        style: { fontSize: "12px", color: "var(--dsw-alias-state-warn-primary,#d29922)", lineHeight: 1.5 },
      }, "⚠️ 重启 DSH 后更新才会生效") : null,
      React.createElement("button", {
        type: "button",
        disabled: !writable || upStatus.status === "checking" || upStatus.status === "updating",
        onClick: function () { requestUpdate("check"); },
        style: btnRowStyle,
      }, upStatus.status === "checking" ? "检测中…" : "检查更新"),
      upStatus.status === "available" ? React.createElement("button", {
        type: "button",
        disabled: !writable || upStatus.status === "updating",
        onClick: function () { requestUpdate("update"); },
        style: Object.assign({}, btnRowStyle, {
          background: "var(--dsw-alias-state-business-primary,#3b82f6)", color: "#fff",
        }),
...
// 画布标签的 iframe + 顶栏持久实例（2026-09-25）：标签切换 = 组件卸载/重挂，
// 若每次新建 iframe 会把 ComfyUI 整个重载。改为模块级常驻，重挂只 re-attach
// （appendChild 移动已存在节点不会重载），首次创建时设定 src 与顶栏。
var TAB_CANVAS = null;
...
function CanvasTab(props) {
  ...
  React.useEffect(function () {
    ...
    if (TAB_CANVAS) {
      ...
      frame.title = "ComfyUI";
      frame.src = activeBase;
      frame.style.cssText = "position:absolute;left:0;right:0;top:34px;bottom:0;width:100%;height:calc(100% - 34px);border:none;display:block;background:#fff;";
      bar = document.createElement("div");
      bar.style.cssText = "position:absolute;top:0;left:0;right:0;height:34px;display:flex;align-items:center;gap:12px;padding:0 12px;background:var(--dsw-alias-bg-layer-1,#232324);border-bottom:1px solid var(--dsw-alias-border-l1,rgba(128,128,128,.25));z-index:5;font-size:13px;";
      var title = document.createElement("strong");
      title.textContent = "ComfyUI";
      bar.appendChild(title);
      ...
      TAB_CANVAS = { frame: frame, bar: bar };
    }
    host.appendChild(frame);
    host.appendChild(bar);
    var sync = function () { if (frame.src !== activeBase) frame.src = activeBase; };
    ...
    return function () {
      ...
      // 只摘除不销毁：iframe 文档保持存活，下次进标签秒开不重载。
...
function apply(ctx) {
  ...
  document.addEventListener("dsh-comfyui-control", function (e) {
    var action = e && e.detail && e.detail.action === "restart" ? "restart" : "stop";
    if (scope && typeof scope.set === "function") {
      scope.set("comfyControl", { requested: true, action: action }).catch(function () { /* recover handled by scope */ });
    }
  });
