...
/**
 * P1: extract a structured execution error from a ComfyUI history item, so a
 * failed run reports "node 7 CheckpointLoaderSimple → FileNotFoundError" instead
 * of a raw JSON wall. ComfyUI stores it under status.execution_error (newer) or
 * status.messages[] entries of type "execution_error" (classic). Returns null
 * when the run had no execution error.
 */
function extractExecutionError(item) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (targetDir && files.length > 0) {
        ...
        // append this run's trace to runs.json (create if missing). Dedup by
        // promptId: re-downloading the same run updates its entry instead of
        // appending a duplicate; `run` numbers continue from the max seen.
        ...
        const existing = runs.findIndex((r) => r && r.promptId === args.promptId)
        ...
        const entry = {
          run: existing >= 0 ? runs[existing].run : (runs.reduce((m, r) => Math.max(m, r?.run || 0), 0) + 1),
        ...
        if (existing >= 0) runs[existing] = entry
