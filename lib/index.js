...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      // Probe with the correct token and a deliberately wrong one: both
      // succeeding means auth is NOT effective on the wire.
      const token = (value?.bridgeToken || '').trim()
      let bridgeAuthEffective = null
      let bridgeAuthProbe = 'not-probed'
      try {
        const good = await fetch(`${baseUrl()}/dsh-bridge/workflow`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
          signal: AbortSignal.timeout(4000),
        })
        const bad = token
          ? await fetch(`${baseUrl()}/dsh-bridge/workflow`, {
            headers: { Authorization: 'Bearer dsh-bt-__wrong-probe__' },
            signal: AbortSignal.timeout(4000),
          })
          : good
        if (!good.ok) {
          bridgeAuthProbe = `unreachable-or-error: HTTP ${good.status}`
        } else if (!token) {
          // No token configured on either side — open by design but flag it.
          bridgeAuthEffective = false
          bridgeAuthProbe = 'no token configured on DSH side'
        } else {
          bridgeAuthEffective = bad.status === 401
          bridgeAuthProbe = bad.status === 401
            ? 'bridge rejects bad token (auth effective)'
            : 'bridge accepted a WRONG token — auth NOT effective on the ComfyUI side'
        }
      } catch (e) {
        bridgeAuthProbe = `probe failed: ${e instanceof Error ? e.message : String(e)}`
      }
      return {
        baseUrl: baseUrl(),
        activeView,
