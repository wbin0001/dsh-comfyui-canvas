...
export function apply(ctx) {
  ...
  // can answer "which parameters produced this image".
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (result && result.promptId) {
        runLedger.set(String(result.promptId), {
          overrides: Array.isArray(args.overrides) ? args.overrides : [],
          timestamp: new Date().toISOString(),
          status: result.nodeErrors ? 'error' : 'queued',
        })
      }
      return result
    },
    ...
    async execute(args, exec) {
      ...
      if (result && Array.isArray(result.results)) {
        for (let i = 0; i < result.results.length; i++) {
          ...
          if (r && r.promptId) {
            runLedger.set(String(r.promptId), {
              overrides,
              timestamp: new Date().toISOString(),
              status: r.nodeErrors ? 'error' : 'queued',
            })
          }
        }
      }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (targetDir && files.length > 0) {
        ...
        // append this run's trace to runs.json (create if missing)
