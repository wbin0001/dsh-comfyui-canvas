...
const ConfigSchema = z.object({
   ...
   */
  probeRequested: z.boolean().default(false),
  ...
  bridgeProbe: z.object({
    ...
    checkedAt: z.number(),
  }).default({ bridgeAuthEffective: null, bridgeAuthProbe: 'not-probed', checkedAt: 0 }),
  /**
  ...
  }).default({ requested: false, action: 'check' }),
  ...
  pluginUpdate: z.object({
    ...
    checkedAt: z.number(),
    updatedAt: z.number(),
  }).default({ status: 'idle', latestVersion: '', message: '', checkedAt: 0, updatedAt: 0 }),
  /**
  ...
  }).default({ requested: false, action: 'stop' }),
  ...
  comfyControlResult: z.object({
    ...
    message: z.string(),
    at: z.number(),
  }).default({ status: 'idle', message: '', at: 0 }),
})
...
export function apply(ctx) {
  ...
  // Assigned after a successful settings.register; no-op until then.
  let safeUpdate = () => undefined
  // The background process handle of the ComfyUI launched by this plugin, so
  // the canvas header's stop/restart buttons can kill it directly.
  let comfyProc = null
