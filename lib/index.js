...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Validate the current ComfyUI canvas. Checks the live graph for structural problems ' +
      '(missing required inputs, bad links) without running it, flashes the offending nodes, ' +
      'and returns the error map.',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'validate', {}, exec.signal, { timeoutMs: 8000 })
        .then(async (res) => {
          ...
          return { nodeErrors: errors, highlighted: ids }
        })
    },
  }))
  ...
  // by session id, and this tool reads the value for the CURRENT session (the
  // agent's initiator session). 'canvas' = the user has the ComfyUI split
  // canvas tab selected for this session right now; 'chat' = plain chat tab.
  ctx.tools.register(defineTool({
    name: 'comfyui_config',
    ...
    async execute() {
      let activeView = 'chat'
      try {
        // Current agent initiator session id — the session this turn belongs to.
        const currentSessionId = ctx.get?.('agents')?.currentInitiator?.()?.session?.id
        const value = settingsScope?.get?.()
        if (currentSessionId && value?.activeViewBySession?.[currentSessionId] === 'canvas') {
          activeView = 'canvas'
        }
      } catch { /* settings unavailable — default to chat */ }
      const value = settingsScope?.get?.()
      return {
        baseUrl: baseUrl(),
        activeView,
