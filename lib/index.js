        ...
        void (async () => {
          ...
          try {
            ...
            if (!killed && shell && typeof shell.start === 'function') {
              // Fallback: kill whatever listens on the configured port.
              let port = '8188'
              try { port = new URL(resolveBase()).port || '8188' } catch { /* keep default */ }
              const killer = shell.start(shell.resolve({
                command: `powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue }"`,
                sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
              }))
