...
import { mediaTypeOf, extractExecutionError, nextStemNumber, upsertRun } from './utils.js'
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const result = await sendCommand(baseUrl(), bridgeToken(), 'batch_run', { runs: args.runs }, exec.signal, { timeoutMs: 30000 })
      // v0.1.3: record every queued run for .runs.json traceability.
      if (result && Array.isArray(result.results)) {
        for (let i = 0; i < result.results.length; i++) {
          const r = result.results[i]
          const overrides = Array.isArray(args.runs?.[i]?.overrides) ? args.runs[i].overrides : []
          if (r && r.promptId) {
