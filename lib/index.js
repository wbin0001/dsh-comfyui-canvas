/**
 * DSH-ComfyUI-Canvas — host-side agent tools.
 *
 * Registers the canvas tools the agent uses to operate the live ComfyUI
 * canvas through ComfyUI-DSH-Canvas bridge (a custom_node injected into
 * ComfyUI). Reads go straight to the bridge's /dsh-bridge/workflow store;
 * writes POST a command, then poll the frontend's execution result.
 *
 * M1: read_workflow + add_node / connect / set_param / remove_node.
 * M2: load_workflow (rewrite loop) + run + debug (validate + highlight).
 * M3: settings-driven configuration — the base URL is read from the
 *     `dsh-comfyui-canvas` settings namespace (registration optional so an
 *     environment without the settings service still starts with defaults).
 */
import { defineTool } from '@deepseek-ai/dsh-tools'
import z from '@deepseek-ai/schemastery'
import path from 'node:path'
import { readFileSync, existsSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { mediaTypeOf, extractExecutionError, nextStemNumber, upsertRun, expandBatchMatrix } from './utils.js'

export const name = 'dsh-comfyui-canvas'
export const inject = ['tools', 'settings', 'shell', 'skills']

const NAMESPACE = 'dsh-comfyui-canvas'
const DEFAULT_BASE = 'http://127.0.0.1:8188'
const ENV_BASE = (process.env.COMFYUI_URL || '').replace(/\/+$/, '')

const ConfigSchema = z.object({
  /** ComfyUI HTTP server base URL. */
  baseUrl: z.string().default(DEFAULT_BASE).volatile(),
  /** Optional port shortcut; when non-empty it rewrites the base URL's port. */
  port: z.number().volatile(),
  /** loopback | lan | cloud-selfhosted | saas */
  networkMode: z.string().default('loopback').volatile(),
  /** Launch command used by the canvas Start button / agent auto-start. */
  launchCommand: z.string().default('').volatile(),
  /**
   * Absolute path to the ComfyUI install directory. Used by comfyui_upgrade
   * (git pull core + custom nodes) and by the launch button's working dir.
   * Empty falls back to the launch command's own directory / env COMfyUI_DIR.
   */
  comfyuiDir: z.string().default('').volatile(),
  /** Split-rail width in px. */
  railWidth: z.number().default(360).volatile(),
  /** Canvas scale percent (legacy UI field; kept in schema so settings don't drop it). */
  canvasScale: z.number().default(80).volatile(),
  /** Last active view across sessions (legacy echo of activeViewBySession). */
  activeView: z.string().default('chat').volatile(),
  /**
   * Project directory for outputs/export/traceability. When empty, downloads
   * and exports default to `<DSH workspace>/projects/<workflow_name>/`.
   * Users with their own organization may point it anywhere (e.g. D:\AI-Projects).
   */
  projectDir: z.string().default('').volatile(),
  /**
   * Optional shared secret for the ComfyUI bridge. When set, every agent
   * request carries `Authorization: Bearer <token>`, and the bridge only
   * answers requests that present the same token (the bridge reads it from
   * its OWN `DSH_BRIDGE_TOKEN` env var). Empty keeps the bridge open, the
   * same trust model as ComfyUI's own /prompt. Keep both sides identical.
   */
  bridgeToken: z.string().default('').volatile(),
  /**
   * One-shot launch request: the client start button sets this true, the host
   * watcher picks it up, starts ComfyUI via launchCommand, then clears it.
   */
  launchRequested: z.boolean().default(false).volatile(),
  /**
   * Last launch failure message (host writes it when the launch command exits
   * before ComfyUI comes up). The client start card shows it instead of
   * hanging on "正在启动…" forever. Cleared on the next successful launch.
   */
  launchError: z.string().default('').volatile(),
  /**
   * Per-session record of which conversation view is active in the browser
   * right now, keyed by session id: 'canvas' = the ComfyUI canvas tab is
   * selected for that session, 'chat' = the plain Chat tab is. Written by the
   * client view component on mount/unmount for ITS OWN session only, so one
   * session's canvas state never leaks into another. comfyui_config reads the
   * current session's value so the agent knows whether to focus on canvas work.
   */
  activeViewBySession: z.dict(z.union([z.const('canvas'), z.const('chat')])).default({}).volatile(),
  /** Per-session split-screen on/off (client canvas split toggle), keyed by session id. */
  splitEnabledBySession: z.dict(z.boolean()).default({}).volatile(),
  /** Per-session collapsed state of the split conversation rail, keyed by session id. */
  railClosedBySession: z.dict(z.boolean()).default({}).volatile(),
  /**
   * One-shot probe request: the client settings page / canvas tab sets this
   * true when it wants a fresh bridge-auth handshake; the host watcher runs
   * the probe (host side has no CORS problem) and writes the result into
   * `bridgeProbe`, then clears this flag. Kept out of the startup path —
   * probing is on-demand, async, and never blocks DSH boot.
   */
  probeRequested: z.boolean().default(false).volatile(),
  /** Last bridge-auth probe result, written by the host watcher. */
  bridgeProbe: z.object({
    bridgeAuthEffective: z.union([z.boolean(), z.const(null)]),
    bridgeAuthProbe: z.string(),
    checkedAt: z.number(),
  }).default({ bridgeAuthEffective: null, bridgeAuthProbe: 'not-probed', checkedAt: 0 }).volatile(),
  /**
   * This plugin's own version, published to settings at startup so the
   * settings page can show it next to the update button.
   */
  pluginVersion: z.string().default('').volatile(),
  /**
   * One-shot plugin-update request from the settings page: the client sets
   * `{ requested: true, action: 'check' | 'update' }`, the host watcher runs
   * the npm-registry check or the package install and writes the outcome into
   * `pluginUpdate`, then clears `requested`. Installs require a DSH restart to
   * take effect, which the client surfaces after a successful update.
   */
  updateRequest: z.object({
    requested: z.boolean(),
    action: z.union([z.const('check'), z.const('update')]),
  }).default({ requested: false, action: 'check' }).volatile(),
  /** Plugin-update state, written by the host watcher for the settings page. */
  pluginUpdate: z.object({
    status: z.string(), // idle | checking | up-to-date | available | updating | done | error
    latestVersion: z.string(),
    message: z.string(),
    checkedAt: z.number(),
    updatedAt: z.number(),
  }).default({ status: 'idle', latestVersion: '', message: '', checkedAt: 0, updatedAt: 0 }).volatile(),
  /**
   * One-shot ComfyUI control request from the canvas header: the client sets
   * `{ requested: true, action: 'stop' | 'restart' }`, the host watcher kills
   * the recorded launch process (falling back to whatever listens on the
   * configured port) and, for 'restart', re-enters the launch path.
   */
  comfyControl: z.object({
    requested: z.boolean(),
    action: z.union([z.const('stop'), z.const('restart')]),
  }).default({ requested: false, action: 'stop' }).volatile(),
  /** Last control outcome, written by the host watcher. */
  comfyControlResult: z.object({
    status: z.string(), // idle | stopping | restarting | done | error
    message: z.string(),
    at: z.number(),
  }).default({ status: 'idle', message: '', at: 0 }).volatile(),
})

/** Entry Config — profile-backed live settings (0.1.7 新核心：describe/schema 由此投影)。 */
export const Config = ConfigSchema

function renderJSON(_args, value) {
  return [{ type: 'text', text: JSON.stringify(value, null, 2) }]
}

/** Extra headers for bridge requests; carries the optional shared token. */
function bridgeHeaders(token) {
  return token
    ? { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }
    : { 'Content-Type': 'application/json' }
}

/**
 * Read the bridge version from a ComfyUI-DSH-Canvas directory: version.json
 * first (authoritative), then fall back to parsing __version__ from
 * __init__.py for older copies. Returns null when neither is readable.
 * @param root - the bridge node directory.
 * @returns the version string, or null.
 */
function readVersionOf(root) {
  try {
    const vj = path.join(root, 'version.json')
    if (existsSync(vj)) {
      const parsed = JSON.parse(readFileSync(vj, 'utf8'))
      if (typeof parsed?.version === 'string' && parsed.version) return parsed.version
    }
    const py = path.join(root, '__init__.py')
    if (existsSync(py)) {
      const m = /__version__\s*=\s*["']([^"']+)["']/.exec(readFileSync(py, 'utf8'))
      if (m) return m[1]
    }
    return null
  } catch {
    return null
  }
}

/**
 * Semver comparison supporting numeric and rc/beta/alpha prerelease segments
 * (rc.10 > rc.9). Returns -1/0/1. Undecidable inputs (missing, non-semver)
 * compare as 0 (equal) so a caller never blocks on an unreadable version.
 * @param a - first version string.
 * @param b - second version string.
 * @returns -1 when a < b, 0 when equal/undecidable, 1 when a > b.
 */
function compareVersions(a, b) {
  const parse = (v) => {
    if (typeof v !== 'string') return null
    const m = /^(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?$/.exec(v.trim())
    if (!m) return null
    return { major: +m[1], minor: +m[2], patch: +m[3], pre: m[4] ? m[4].split('.') : [] }
  }
  const pa = parse(a)
  const pb = parse(b)
  if (!pa || !pb) return 0
  for (const key of ['major', 'minor', 'patch']) {
    if (pa[key] !== pb[key]) return pa[key] < pb[key] ? -1 : 1
  }
  if (pa.pre.length === 0 && pb.pre.length === 0) return 0
  if (pa.pre.length === 0) return 1
  if (pb.pre.length === 0) return -1
  for (let i = 0; i < Math.max(pa.pre.length, pb.pre.length); i++) {
    const sa = pa.pre[i]
    const sb = pb.pre[i]
    if (sa === undefined) return -1
    if (sb === undefined) return 1
    const na = Number(sa)
    const nb = Number(sb)
    const naOk = Number.isFinite(na) && /^\d+$/.test(sa)
    const nbOk = Number.isFinite(nb) && /^\d+$/.test(sb)
    if (naOk && nbOk) {
      if (na !== nb) return na < nb ? -1 : 1
    } else if (sa !== sb) {
      return sa < sb ? -1 : 1
    }
  }
  return 0
}

/**
 * Expand the batch_run `matrix` form into concrete runs (override-set arrays),
 * or pass `runs` through unchanged. Exactly one of the two must be present.
 * - matrix.mode 'zip': parallel slots — the i-th value of every field forms
 *   run i (length = the shortest field); every field must have a value for
 *   that slot, so unequal lengths are an error rather than a silent crop.
 * - matrix.mode 'product': cartesian product — every combination of one value
 *   per field (length = product of field lengths).
 * @param args - the tool args ({runs} or {matrix}).
 * @returns the concrete runs array passed to the bridge.
 */
/**
 * P0 handshake: does the BRIDGE actually enforce the token? The DSH-side
 * setting alone proves nothing — if the ComfyUI side never got
 * DSH_BRIDGE_TOKEN, _authorized() always passes and the bridge is open.
 * Probe with the correct token and a deliberately wrong one: both succeeding
 * means auth is NOT effective on the wire. Shared by comfyui_config and the
 * settings watcher (the client cannot probe cross-origin, so the host probes
 * on the client's behalf).
 */
async function probeBridgeAuth(base, token) {
  const result = {
    bridgeAuthEffective: null,
    bridgeAuthProbe: 'not-probed',
    checkedAt: Date.now(),
  }
  try {
    const good = await fetch(`${base}/dsh-bridge/workflow`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      signal: AbortSignal.timeout(4000),
    })
    const bad = token
      ? await fetch(`${base}/dsh-bridge/workflow`, {
        headers: { Authorization: 'Bearer dsh-bt-__wrong-probe__' },
        signal: AbortSignal.timeout(4000),
      })
      : good
    if (!good.ok) {
      result.bridgeAuthProbe = `unreachable-or-error: HTTP ${good.status}`
    } else if (!token) {
      // No token configured on either side — open by design but flag it.
      result.bridgeAuthEffective = false
      result.bridgeAuthProbe = 'no token configured on DSH side'
    } else {
      result.bridgeAuthEffective = bad.status === 401
      result.bridgeAuthProbe = bad.status === 401
        ? 'bridge rejects bad token (auth effective)'
        : 'bridge accepted a WRONG token — auth NOT effective on the ComfyUI side'
    }
  } catch (e) {
    result.bridgeAuthProbe = `probe failed: ${e instanceof Error ? e.message : String(e)}`
  }
  return result
}

/** POST a write command to the bridge and poll for the frontend result. */
async function sendCommand(baseUrl, token, cmd, payload, signal, { pollMs = 400, timeoutMs = 8000 } = {}) {
  const started = Date.now()
  const res = await fetch(`${baseUrl}/dsh-bridge/command`, {
    method: 'POST',
    headers: bridgeHeaders(token),
    body: JSON.stringify({ cmd, payload }),
    signal,
  })
  if (!res.ok) throw new Error(`bridge command HTTP ${res.status}`)
  const { accepted, id, error } = await res.json()
  if (!accepted) throw new Error(error || 'bridge command not accepted')

  while (Date.now() - started < timeoutMs) {
    const r = await fetch(`${baseUrl}/dsh-bridge/result/${id}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      signal,
    })
    const item = await r.json().catch(() => null)
    if (item && item.error !== 'not_found') {
      if (item.ok === false) throw new Error(item.error || 'command failed')
      return item.result
    }
    await new Promise((done) => setTimeout(done, pollMs))
  }
  throw new Error('bridge command timed out — is the ComfyUI page loaded with the bridge?')
}

// Unwrap schemastery Volatile wrappers: volatile Config fields validate to lazy
// Volatile objects; .get() yields the plain value (mirrors settings' plainConfig).
const plainValue = (value) => {
  if (value && typeof value === 'object' && typeof value.get === 'function' && !Array.isArray(value)) return plainValue(value.get())
  if (Array.isArray(value)) return value.map(plainValue)
  if (value && typeof value === 'object') {
    const out = {}
    for (const key of Object.keys(value)) out[key] = plainValue(value[key])
    return out
  }
  return value
}

export function apply(ctx) {
  // v0.1.10: DSH 0.1.7 新核心 — 配置经 entry Config（profile 支撑）读写：
  // 读 ctx.fiber?.config，写 ctx.settings.update(NAMESPACE, patch)。settingsScope
  // 服务已随核心移除，不再使用。
  const readConfig = () => plainValue(ctx.fiber?.config ?? {})
  let resolveBase = () => {
    const value = readConfig()
    const base = (ENV_BASE || value?.baseUrl || DEFAULT_BASE).replace(/\/+$/, '')
    if (value?.port != null && value.port !== '' && /^\d+$/.test(String(value.port))) {
      return base.replace(/:\d+$/, '') + ':' + String(value.port)
    }
    return base
  }
  // Every settings write is a promise; funnel through a wrapper so an
  // unhandled rejection can never fatal boot (installFailLoud).
  let safeUpdate = () => undefined
  // The background process handle of the ComfyUI launched by this plugin, so
  // stop/restart can kill it directly.
  let comfyProc = null

  // v0.1.3: in-memory run ledger for `.runs.json` traceability — keyed by
  // prompt_id, recording the overrides each run was started with so downloads
  // can answer "which parameters produced this image". Bounded: DSH runs are
  // long-lived and every run adds an entry, so cap it and drop the oldest
  // (Map preserves insertion order).
  const runLedger = new Map()
  const RUN_LEDGER_MAX = 500
  const recordRun = (promptId, overrides, status) => {
    runLedger.set(String(promptId), {
      overrides: Array.isArray(overrides) ? overrides : [],
      timestamp: new Date().toISOString(),
      status,
    })
    if (runLedger.size > RUN_LEDGER_MAX) {
      const oldest = runLedger.keys().next().value
      if (oldest !== undefined) runLedger.delete(oldest)
    }
  }

  // v0.1.3: resolve the project directory — explicit setting wins, otherwise
  // default to `<DSH workspace>/projects`. This is where get_outputs downloads
  // land and `.runs.json` is written, so one project's assets stay together
  // (outputs, export JSON, trace) without touching ComfyUI's own output/.
  const resolveProjectDir = () => {
    const value = readConfig()
    const explicit = (value?.projectDir || '').trim()
    if (explicit) return explicit
    return path.join(process.cwd(), 'projects')
  }

  // 0.1.7 新核心：settingsScope 服务已移除，settings.register 不再存在。
  // 用本地适配器模拟旧 scope 接口 — get() 读 ctx.fiber?.config，update() 写
  // ctx.settings.update(NAMESPACE, patch)，watch() 登记回调（由下方
  // settings/document-updated 订阅驱动）。缺失时保持默认值，不阻塞启动。
  const settings = ctx.settings ?? ctx.get?.('settings')
  if (settings && typeof settings.update === 'function') {
    try {
      const configWatchers = new Set()
      const scope = {
        get: () => plainValue(ctx.fiber?.config ?? {}),
        update: (patch) => settings.update(NAMESPACE, patch),
        watch: (fn) => { configWatchers.add(fn); return () => configWatchers.delete(fn) },
      }
      // Every queued settings write is a promise; try/catch CANNOT see its
      // rejection, and an unhandled rejection is fatal to boot (installFailLoud
      // reports it as "fatal load failure"). Funnel all writes through here.
      safeUpdate = (patch) => {
        try { void Promise.resolve(scope.update(patch)).catch(() => undefined) } catch { /* best effort */ }
      }
      // Launch watcher: the client Start button sets launchRequested=true; we
      // start ComfyUI via launchCommand (background), then clear the flag.
      let launching = false
      scope.watch(() => {
        const value = scope.get()
        if (!value?.launchRequested || launching) return
        launching = true
        void (async () => {
          const fail = (message) => {
            console.error(`[dsh-comfyui-canvas] ${message}`)
            safeUpdate({ launchError: message })
          }
          try {
            const cmd = (value?.launchCommand || '').trim()
            if (!cmd) {
              fail('launch requested but launchCommand is empty')
              return
            }
            // Clear any stale error from a previous attempt before launching.
            safeUpdate({ launchError: '' })
            const workdir = (value?.comfyuiDir || '').trim() || undefined
            const shell = ctx.shell
            if (shell && typeof shell.start === 'function') {
              // ComfyUI lives outside the session workspace (E: drive) and
              // writes output/temp as a long-lived server, so the launch must
              // run unconfined — the default workspace-write sandbox would
              // deny the bat and kill ComfyUI before it ever binds 8188.
              // Pass the same env the DSH launcher sets: custom_nodes
              // PYTHONPATH, TF isolation, venv isolation, bridge token.
              const proc = shell.start(shell.resolve({
                command: cmd,
                ...(workdir ? { workdir } : {}),
                env: {
                  PYTHONPATH: [
                    workdir ? `${workdir}\\custom_nodes` : 'custom_nodes',
                    process.env.PYTHONPATH || '',
                  ].filter(Boolean).join(';'),
                  EINOPS_DISABLE_TF: '1',
                  COMFY_ENV_ISOLATE: 'false',
                  DSH_BRIDGE_TOKEN: String(value?.bridgeToken || process.env.DSH_BRIDGE_TOKEN || ''),
                },
                sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
              }))
              comfyProc = proc
              console.log(`[dsh-comfyui-canvas] launching ComfyUI: ${cmd}`)
              // A healthy launch keeps python alive well past this grace
              // window; an immediate exit (bad command / sandbox denial /
              // crash) is a launch failure the client must see.
              let grace = setTimeout(() => { grace = null }, 30000)
              const settle = (detail) => {
                if (!grace) return // still running after grace = launched OK
                clearTimeout(grace)
                grace = null
                fail(`ComfyUI launch process exited (${detail}) — check the launch command / install dir`)
              }
              proc.done.then(
                (outcome) => settle(`exit ${outcome?.exitCode ?? outcome?.code ?? '?'}`),
                (err) => settle(String(err?.message ?? err)),
              )
            } else {
              fail('no shell service available to launch ComfyUI')
            }
          } finally {
            launching = false
            safeUpdate({ launchRequested: false })
          }
        })()
      })

      // Bridge-auth probe watcher: the client settings page / canvas tab sets
      // probeRequested=true when it wants a fresh handshake; the host probes
      // (host-side fetch has no CORS issue — the browser cannot read ComfyUI's
      // status cross-origin, which is why the client asks us to do it) and
      // writes the verdict back into `bridgeProbe`, then clears the flag.
      let probing = false
      scope.watch(() => {
        const value = scope.get()
        if (!value?.probeRequested || probing) return
        probing = true
        void (async () => {
          try {
            const token = (value?.bridgeToken || '').trim()
            const base = resolveBase()
            const result = await probeBridgeAuth(base, token)
            try {
              safeUpdate({ bridgeProbe: result, probeRequested: false })
            } catch { /* best effort */ }
          } finally {
            probing = false
          }
        })()
      })

      // Publish this plugin's version once so the settings page can show it
      // next to the update button (best effort — never blocks startup).
      try {
        const pkgPath = fileURLToPath(new URL('../package.json', import.meta.url))
        const version = String(JSON.parse(readFileSync(pkgPath, 'utf8')).version || '')
        if (version) safeUpdate({ pluginVersion: version })
      } catch { /* best effort */ }

      // Plugin-update watcher: the settings page sets updateRequest with
      // action 'check' (query npm registry for the latest version) or
      // 'update' (install the latest package into the DSH web profile).
      // Installs run unconfined (the profile lives outside the session
      // workspace) and require a DSH restart to take effect.
      let updatingPlugin = false
      scope.watch(() => {
        const value = scope.get()
        const req = value?.updateRequest
        if (!req?.requested || updatingPlugin) return
        updatingPlugin = true
        void (async () => {
          const setState = (patch) => {
            safeUpdate(patch)
          }
          try {
            const action = req.action === 'update' ? 'update' : 'check'
            const current = String(value?.pluginVersion || '')
            setState({
              updateRequest: { requested: false, action },
              pluginUpdate: {
                status: action === 'update' ? 'updating' : 'checking',
                latestVersion: '',
                message: action === 'update' ? '正在安装最新版本…' : '正在查询最新版本…',
                checkedAt: Date.now(),
                updatedAt: 0,
              },
            })
            const res = await fetch('https://registry.npmjs.org/dsh-comfyui-canvas/latest', {
              signal: AbortSignal.timeout(10000),
            })
            if (!res.ok) throw new Error(`npm registry HTTP ${res.status}`)
            const meta = await res.json().catch(() => null)
            const latest = String(meta?.version || '')
            if (!latest) throw new Error('npm registry returned no version')
            if (action === 'check') {
              const upToDate = latest === current
              setState({
                pluginUpdate: {
                  status: upToDate ? 'up-to-date' : 'available',
                  latestVersion: latest,
                  message: upToDate
                    ? `已是最新版本 v${latest}`
                    : `发现新版本 v${latest}（当前 v${current || '?'}）`,
                  checkedAt: Date.now(),
                  updatedAt: 0,
                },
              })
              return
            }
            // Update: install the latest package into the DSH web profile.
            const shell = ctx.shell
            if (!shell || typeof shell.start !== 'function') throw new Error('no shell service available to update the plugin')
            const profileDir = process.env.DSH_HOME
              ? path.join(process.env.DSH_HOME, 'profiles', 'web')
              : path.dirname(path.dirname(path.dirname(fileURLToPath(import.meta.url))))
            const proc = shell.start(shell.resolve({
              command: 'pnpm add dsh-comfyui-canvas@latest',
              workdir: profileDir,
              sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
            }))
            const outcome = await proc.done
            if (!outcome || (outcome.exitCode ?? outcome.code) !== 0) {
              throw new Error('install failed — see DSH logs')
            }
            setState({
              pluginUpdate: {
                status: 'done',
                latestVersion: latest,
                message: `已更新到 v${latest}，重启 DSH 后生效`,
                checkedAt: Date.now(),
                updatedAt: Date.now(),
              },
            })
          } catch (e) {
            setState({
              pluginUpdate: {
                status: 'error',
                latestVersion: '',
                message: `更新失败：${e instanceof Error ? e.message : String(e)}`,
                checkedAt: Date.now(),
                updatedAt: 0,
              },
            })
          } finally {
            updatingPlugin = false
          }
        })()
      })

      // ComfyUI control watcher: the canvas header's stop/restart buttons set
      // comfyControl with action 'stop' (kill the recorded launch process, or
      // whatever listens on the configured port as a fallback) or 'restart'
      // (kill, then re-enter the launch path). The canvas status dot reflects
      // the outcome through the existing reachability probe.
      let controlling = false
      scope.watch(() => {
        const value = scope.get()
        const ctrl = value?.comfyControl
        if (!ctrl?.requested || controlling) return
        controlling = true
        void (async () => {
          const setState = (patch) => {
            safeUpdate(patch)
          }
          try {
            const action = ctrl.action === 'restart' ? 'restart' : 'stop'
            setState({
              comfyControl: { requested: false, action },
              comfyControlResult: {
                status: action === 'restart' ? 'restarting' : 'stopping',
                message: action === 'restart' ? '正在重启 ComfyUI…' : '正在关闭 ComfyUI…',
                at: Date.now(),
              },
            })
            const shell = ctx.shell
            let killed = false
            if (comfyProc && typeof comfyProc.kill === 'function') {
              killed = comfyProc.kill()
              try { await comfyProc.done } catch { /* already settled */ }
              comfyProc = null
            }
            if (!killed && shell && typeof shell.start === 'function') {
              // Fallback: kill whatever listens on the configured port. Uses
              // netstat + PID parsing because Get-NetTCPConnection can come
              // back empty in some environments even when the port is
              // listening (observed on this host). The command is plain
              // PowerShell: the pwsh-local executor passes it as ONE argv
              // element to `pwsh -Command`, so there is no intermediate shell
              // to double-quote for — wrapping it in another
              // `powershell -NoProfile -Command "..."` would let the outer
              // pwsh interpolate $_ / $pids and break the script.
              let port = '8188'
              try { port = new URL(resolveBase()).port || '8188' } catch { /* keep default */ }
              const killer = shell.start(shell.resolve({
                command: `$pids = netstat -ano | Select-String ':${port}' | Select-String 'LISTENING' | ForEach-Object { ($_ -split '\\s+')[-1] } | Sort-Object -Unique; foreach ($procId in $pids) { Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue }; Write-Output ('killed=' + ($pids | Measure-Object).Count)`,
                sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
              }))
              try { await killer.done } catch { /* best effort */ }
              const out = (typeof killer.readOutput === 'function' ? killer.readOutput().delta : '') || ''
              const matched = /killed=(\d+)/.exec(out)
              killed = matched ? Number(matched[1]) > 0 : false
            }
            console.log(`[dsh-comfyui-canvas] comfyControl ${action}: killed=${killed}`)
            if (action === 'restart') {
              // Give the port a moment to release, then re-enter the launch path.
              await new Promise((done) => setTimeout(done, 2500))
              setState({ launchRequested: true })
              setState({ comfyControlResult: { status: 'done', message: 'ComfyUI 已停止，正在重新启动…', at: Date.now() } })
            } else {
              setState({ comfyControlResult: { status: 'done', message: killed ? 'ComfyUI 已关闭' : '未找到运行中的 ComfyUI', at: Date.now() } })
            }
          } catch (e) {
            setState({ comfyControlResult: { status: 'error', message: `操作失败：${e instanceof Error ? e.message : String(e)}`, at: Date.now() } })
          } finally {
            controlling = false
          }
        })()
      })
      // 0.1.7: drive the registered watchers once at startup, then on every
      // namespace config change (scope.watch 由该订阅驱动，等价旧 watch() 语义)。
      const runConfigWatchers = () => {
        for (const fn of configWatchers) { try { fn() } catch { /* watcher error ignored */ } }
      }
      runConfigWatchers()
      const pollTimer = setInterval(runConfigWatchers, 1500)
      if (typeof ctx.on === 'function') {
        ctx.on('settings/document-updated', (ns) => { if (ns === NAMESPACE) runConfigWatchers() })
      }
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => () => clearInterval(pollTimer))
      }
    } catch (adapterError) {
      // Config adapter unavailable — keep defaults.
    }
  }

  const baseUrl = () => resolveBase()
  const bridgeToken = () => String(readConfig()?.bridgeToken || '')

  ctx.tools.register(defineTool({
    name: 'comfyui_read_workflow',
    description: 'Read the current ComfyUI canvas state: node list summary plus the full workflow JSON. ' +
      'Use before editing the canvas so you know which node ids exist and how they are connected.',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args, exec) {
      // Ask the live frontend to re-report the graph right now, so an open but
      // idle page does not serve a stale cache. Best-effort: if no frontend is
      // loaded (or the bridge is missing), this times out and is ignored.
      await sendCommand(baseUrl(), bridgeToken(), 'refresh_report', {}, exec.signal, { timeoutMs: 2500 })
        .catch(() => null)
      const res = await fetch(`${baseUrl()}/dsh-bridge/workflow`, {
        headers: bridgeToken() ? { Authorization: `Bearer ${bridgeToken()}` } : undefined,
        signal: exec.signal,
      })
      if (!res.ok) throw new Error(`bridge HTTP ${res.status}`)
      const data = await res.json()
      if (!data.updated_at) {
        return {
          ready: false,
          error: 'No ComfyUI frontend has reported canvas state yet — open the ComfyUI canvas page (the bridge reports on load and on every change).',
          nodes: [],
          workflow: null,
          updated_at: null,
        }
      }
      // Normalize node/link ids to numbers. The frontend reports ids as
      // strings; every write tool (connect srcId/dstId, set_param nodeId, …)
      // declares `number`, so returning strings forces the model to convert.
      // Coerce only numeric-looking ids; leave exotic ones untouched.
      const normalized = {
        ...data,
        nodes: Array.isArray(data.nodes) ? data.nodes.map((n) => {
          const id = Number(n?.id)
          return { ...n, id: Number.isFinite(id) ? id : n?.id }
        }) : data.nodes,
        workflow: data.workflow && Array.isArray(data.workflow.nodes)
          ? {
            ...data.workflow,
            nodes: data.workflow.nodes.map((n) => {
              const id = Number(n?.id)
              return { ...n, id: Number.isFinite(id) ? id : n?.id }
            }),
            links: Array.isArray(data.workflow.links) ? data.workflow.links.map((l) => {
              const src = Number(l?.[1])
              const dst = Number(l?.[3])
              return [
                l?.[0],
                Number.isFinite(src) ? src : l?.[1],
                l?.[2],
                Number.isFinite(dst) ? dst : l?.[3],
                ...(l?.slice ? l.slice(4) : []),
              ]
            }) : data.workflow.links,
          }
          : data.workflow,
      }
      return { ready: true, ...normalized }
    },
  }))

  ctx.tools.register(defineTool({
    name: 'comfyui_add_node',
    description: 'Add a node to the live ComfyUI canvas. `class` is the ComfyUI node class (e.g. "KSampler", "VAEDecode"). ' +
      '`pos` is an optional [x, y] canvas position. The node appears on the canvas immediately.',
    parameters: {
      class: { type: 'string', required: true, description: 'ComfyUI node class name' },
      pos: { type: 'array', description: 'Optional [x, y] canvas position' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'add_node', { class: args.class, pos: args.pos }, exec.signal)
    },
  }))

  ctx.tools.register(defineTool({
    name: 'comfyui_connect',
    description: 'Connect two nodes on the live ComfyUI canvas: `srcId:srcSlot` to `dstId:dstSlot`. ' +
      'Slots are output/input indexes; use comfyui_read_workflow to see exact ids.',
    parameters: {
      srcId: { type: 'number', required: true, description: 'Source node id' },
      srcSlot: { type: 'number', required: true, description: 'Source output slot index' },
      dstId: { type: 'number', required: true, description: 'Destination node id' },
      dstSlot: { type: 'number', required: true, description: 'Destination input slot index' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'connect', {
        srcId: args.srcId, srcSlot: args.srcSlot, dstId: args.dstId, dstSlot: args.dstSlot,
      }, exec.signal)
    },
  }))

  ctx.tools.register(defineTool({
    name: 'comfyui_set_param',
    description: 'Set a widget value on a node in the live ComfyUI canvas (e.g. a prompt text, seed, or steps).',
    parameters: {
      nodeId: { type: 'number', required: true, description: 'Node id' },
      key: { type: 'string', required: true, description: 'Widget name, e.g. "text", "seed", "steps"' },
      value: { type: 'json', description: 'New widget value (string, number, or boolean)' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'set_param', { nodeId: args.nodeId, key: args.key, value: args.value }, exec.signal)
    },
  }))

  ctx.tools.register(defineTool({
    name: 'comfyui_remove_node',
    description: 'Remove a node (and its connections) from the live ComfyUI canvas.',
    parameters: {
      nodeId: { type: 'number', required: true, description: 'Node id to remove' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'remove_node', { nodeId: args.nodeId }, exec.signal)
    },
  }))

  // M2: rewrite loop — swap the whole canvas for a workflow JSON.
  ctx.tools.register(defineTool({
    name: 'comfyui_load_workflow',
    description: 'Replace the entire live ComfyUI canvas with a workflow JSON (UI format, as returned by ' +
      'comfyui_read_workflow). Use to apply a full rewrite: read → modify node list/links → load back.',
    parameters: {
      workflow: { type: 'json', required: true, description: 'Workflow JSON (UI format with nodes/links). May also be the object from comfyui_read_workflow.workflow.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const data = args.workflow
      // A full comfyui_read_workflow result carries `nodes`/`prompt`/`updated_at`
      // alongside the actual UI workflow under `workflow`. When the caller passes
      // that whole object (the documented usage), unwrap to its `.workflow`
      // instead of feeding the read envelope to the bridge.
      const isReadResult = data && typeof data === 'object' && !Array.isArray(data)
        && Array.isArray(data.nodes)
        && data.workflow && typeof data.workflow === 'object'
      const wf = isReadResult ? data.workflow : data
      return sendCommand(baseUrl(), bridgeToken(), 'load_workflow', { workflow: wf }, exec.signal, { timeoutMs: 15000 })
    },
  }))

  // M2: run the current canvas graph and return the prompt id.
  // `overrides` temporarily sets widget values (e.g. seed/prompt) for this run
  // only — the canvas is restored afterwards, so the same graph can be swept.
  ctx.tools.register(defineTool({
    name: 'comfyui_run',
    description: 'Run the current ComfyUI canvas graph (queuePrompt) and return the prompt id. ' +
      'Optionally pass `overrides` ([{nodeId, key, value}]) to run with temporary widget values ' +
      '(e.g. seed/prompt/strength) — the canvas is restored afterwards, leaving it untouched. ' +
      'After running, poll results with comfyui_get_outputs(promptId).',
    parameters: {
      overrides: { type: 'array', description: 'Optional [{nodeId, key, value}] temporary widget values for this run only.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const result = await sendCommand(baseUrl(), bridgeToken(), 'run', { overrides: args.overrides }, exec.signal, { timeoutMs: 20000 })
      // v0.1.3: record the run for .runs.json traceability.
      if (result && result.promptId) {
        recordRun(result.promptId, args.overrides, result.nodeErrors ? 'error' : 'queued')
      }
      // P1: a submission failure (nodeErrors) often hides a concrete execution
      // error. Best-effort fetch /history right away to surface it structurally.
      const hasNodeErrors = result && result.nodeErrors && Object.keys(result.nodeErrors).length > 0
      if (result && result.promptId && hasNodeErrors) {
        try {
          const pid = encodeURIComponent(result.promptId)
          const hr = await fetch(`${baseUrl()}/history/${pid}`, {
            signal: AbortSignal.any([exec.signal, AbortSignal.timeout(8000)]),
          })
          if (hr.ok) {
            const history = await hr.json()
            const executionError = extractExecutionError(history?.[result.promptId])
            if (executionError) result.executionError = executionError
          }
        } catch { /* best effort — nodeErrors alone is still actionable */ }
      }
      return result
    },
  }))

  // M4: batch run — sweep the current graph over a parameter matrix.
  // Each run in `runs` is an override set; every run is queued as its own
  // prompt and returns a prompt_id. ComfyUI executes the queue serially.
  ctx.tools.register(defineTool({
    name: 'comfyui_batch_run',
    description: 'Batch-run the current ComfyUI graph over a parameter matrix. ' +
      'Two input forms (use exactly one): `runs` — a list of override sets, e.g. ' +
      '[{overrides: [{nodeId, key, value}]}], one per run; or `matrix` — declarative ' +
      'sweep of fields: {fields: [{nodeId, key, values: [...]}], mode: "zip"|"product"}, ' +
      'auto-expanded into one run per combination (zip = parallel slots, product = cartesian). ' +
      'Each run is queued as its own prompt (ComfyUI executes serially) and returns its prompt_id. ' +
      'The canvas is restored after every run. Collect results with comfyui_get_outputs per id.',
    parameters: {
      runs: {
        type: 'array',
        description: 'Override sets, one per run. Each is [{nodeId, key, value}] temporary widget values. Use this OR matrix, not both.',
      },
      matrix: {
        type: 'object',
        // Required by the tool-schema compiler: object params must declare
        // additionalProperties explicitly (see packages/core/tools/src/schema.ts).
        additionalProperties: true,
        description: 'Declarative parameter sweep: {fields: [{nodeId, key, values}], mode: "zip"|"product"}. Expanded into runs. Use this OR runs, not both.',
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      // v0.1.6: expand the matrix form into concrete runs before dispatch.
      const runs = expandBatchMatrix(args)
      const result = await sendCommand(baseUrl(), bridgeToken(), 'batch_run', { runs }, exec.signal, { timeoutMs: 30000 })
      // v0.1.3: record every queued run for .runs.json traceability.
      if (result && Array.isArray(result.results)) {
        for (let i = 0; i < result.results.length; i++) {
          const r = result.results[i]
          const overrides = Array.isArray(runs?.[i]?.overrides) ? runs[i].overrides : []
          if (r && r.promptId) {
            recordRun(r.promptId, overrides, r.nodeErrors ? 'error' : 'queued')
          }
        }
      }
      return result
    },
  }))

  // M2: fetch the output files of a completed run (images, videos, gifs,
  // audio). Reads ComfyUI's own /history/{promptId} (native endpoint, same
  // trust model as /prompt) and lists every output node's files (kind /
  // filename / subfolder / type / view URL). With downloadDir, the files are
  // also saved locally. `ready:false` means the id is not in history yet.
  ctx.tools.register(defineTool({
    name: 'comfyui_get_outputs',
    description: 'Fetch the output files produced by a completed ComfyUI run (comfyui_run returns prompt_id): ' +
      'images, videos, gifs, and audio. Reads ComfyUI /history/{promptId}, lists every output node with its ' +
      'files (kind/filename/type/subfolder/URL), and downloads them into downloadDir — or the project directory ' +
      '(Settings → 项目目录, default <workspace>/projects) when downloadDir is omitted. With outputStem, files are ' +
      'saved as <stem>.<NN>.<ext> with an auto-incrementing number so sweep runs never overwrite each other; ' +
      'each download also appends the run trace (promptId/overrides/timestamp/files) to runs.json. ' +
      'Returns ready:false while the run is still executing.',
    parameters: {
      promptId: { type: 'string', required: true, description: 'The prompt_id returned by comfyui_run.' },
      downloadDir: { type: 'string', description: 'Optional absolute local directory to save the output files into. Defaults to the project directory when omitted.' },
      outputStem: { type: 'string', description: 'Optional filename stem; files are saved as <stem>.<NN>.<ext> (NN auto-increments, never overwrites).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const base = baseUrl()
      const pid = encodeURIComponent(args.promptId)
      // Timeout the history read so a hung ComfyUI endpoint can't strand the
      // agent forever; keep exec.signal (agent cancel) in the same signal.
      const res = await fetch(`${base}/history/${pid}`, {
        signal: AbortSignal.any([exec.signal, AbortSignal.timeout(15000)]),
      })
      if (!res.ok) throw new Error(`history HTTP ${res.status}`)
      const history = await res.json()
      const item = history?.[args.promptId]
      if (!item) {
        return { promptId: args.promptId, ready: false, error: 'not in history yet — run may still be executing, or the id is wrong' }
      }
      const outputs = item.outputs ?? {}
      const files = []
      // ComfyUI history outputs carry file arrays under per-kind keys:
      // images / videos / gifs / audio (depends on the node). Scan them all so
      // a run's video/audio/3D outputs come back like images do — each entry
      // keeps its kind + a /view URL (the view endpoint serves any stored file).
      const FILE_KINDS = ['images', 'videos', 'gifs', 'audio']
      // Singular label per history key (images→image, videos→video, gifs→gif,
      // audio→audio). A naive slice(0,-1) would corrupt "audio" into "audi".
      const KIND_LABEL = { images: 'image', videos: 'video', gifs: 'gif', audio: 'audio' }
      for (const [nodeId, out] of Object.entries(outputs)) {
        for (const kind of FILE_KINDS) {
          const list = Array.isArray(out?.[kind]) ? out[kind] : []
          for (const file of list) {
            const filename = file?.filename
            if (!filename) continue
            const subfolder = file?.subfolder ?? ''
            const type = file?.type ?? 'output'
            const qs = `filename=${encodeURIComponent(filename)}` +
              (subfolder ? `&subfolder=${encodeURIComponent(subfolder)}` : '') +
              `&type=${encodeURIComponent(type)}`
            files.push({ nodeId, kind: KIND_LABEL[kind] ?? kind, filename, subfolder, type, url: `${base}/view?${qs}` })
          }
        }
      }
      // v0.1.3: downloads land in downloadDir, or the project directory
      // (explicit setting, defaulting to <workspace>/projects). outputStem
      // renames to <stem>.<NN>.<ext> so sweep runs never overwrite; the run
      // ledger (promptId/overrides/timestamp from comfyui_run/batch_run) is
      // appended to runs.json for traceability.
      let downloaded = []
      let runsFile = null
      const targetDir = args.downloadDir || resolveProjectDir()
      if (targetDir && files.length > 0) {
        const fs = await import('node:fs')
        const path = await import('node:path')
        fs.mkdirSync(targetDir, { recursive: true })
        // next NN for outputStem: scan existing <stem>.<NN>.<ext> in targetDir
        const stem = (args.outputStem || '').trim()
        let nextNum = 1
        if (stem) {
          let existing = []
          try { existing = fs.readdirSync(targetDir) } catch { /* dir may not exist yet */ }
          nextNum = nextStemNumber(existing, stem)
        }
        const savedNames = []
        for (const f of files) {
          const resp = await fetch(f.url, {
            signal: AbortSignal.any([exec.signal, AbortSignal.timeout(30000)]),
          })
          if (!resp.ok) throw new Error(`download ${f.filename} HTTP ${resp.status}`)
          const buf = Buffer.from(await resp.arrayBuffer())
          const ext = path.extname(f.filename) || '.bin'
          const localName = stem
            ? `${stem}.${String(nextNum).padStart(2, '0')}${ext}`
            : f.filename
          if (stem) nextNum++
          const local = path.join(targetDir, localName)
          fs.writeFileSync(local, buf)
          downloaded.push({ ...f, local, localName })
          savedNames.push(localName)
        }
        // append this run's trace to runs.json (create if missing). Upsert by
        // promptId (pure helper): re-downloading the same run updates its entry
        // instead of appending a duplicate; `run` numbers continue from max.
        const ledger = runLedger.get(String(args.promptId)) || {}
        const runsPath = path.join(targetDir, 'runs.json')
        let runs = []
        try {
          if (fs.existsSync(runsPath)) runs = JSON.parse(fs.readFileSync(runsPath, 'utf8'))
          if (!Array.isArray(runs)) runs = []
        } catch { runs = [] }
        // Prefer the run's FINAL history status ('success'/'error') over the
        // ledger's provisional 'queued' — the ledger is written at submit time,
        // history reflects what actually happened.
        const finalStatus = item.status?.status_str || ledger.status || 'success'
        const entry = {
          promptId: args.promptId,
          overrides: Array.isArray(ledger.overrides) ? ledger.overrides : [],
          timestamp: ledger.timestamp || new Date().toISOString(),
          status: finalStatus,
          files: savedNames,
        }
        runs = upsertRun(runs, entry)
        fs.writeFileSync(runsPath, JSON.stringify(runs, null, 2))
        runsFile = runsPath
      }
      return {
        promptId: args.promptId,
        ready: true,
        status: item.status?.status_str ?? null,
        completed: item.status?.completed ?? false,
        nodeCount: Object.keys(outputs).length,
        outputs: files,
        downloaded,
        runsFile,
        // P1: structured execution error when the run failed on a node, so the
        // agent can name the failing node/exception instead of reading raw JSON.
        executionError: extractExecutionError(item),
      }
    },
  }))

  // M2: debug — validate the graph and flash offending nodes.
  // Uses the bridge's validate command (local structural checks only), which
  // never submits /prompt, so debug has no side effect on a valid graph.
  // Note: validate flags nodes with unconnected REQUIRED inputs; type-mismatch
  // links cannot survive — LiteGraph's connect() rejects them at connect time.
  // Dynamic-slot nodes (switch/index, numbered inputs like image15) report as
  // `warnings` instead of errors, so debug does not false-flag expected slots.
  ctx.tools.register(defineTool({
    name: 'comfyui_debug',
    description: 'Validate the current ComfyUI canvas. Checks the live graph for structural problems ' +
      '(unconnected required inputs) without running it, flashes the offending nodes, ' +
      'and returns the error map. Dynamic-slot nodes (switch/index selectors with numbered inputs ' +
      'such as image15) are reported as warnings, not errors.',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'validate', {}, exec.signal, { timeoutMs: 8000 })
        .then(async (res) => {
          const errors = res?.nodeErrors ?? {}
          const warnings = res?.warnings ?? {}
          const ids = res?.offendingIds ?? Object.keys(errors)
          if (ids.length > 0) {
            await sendCommand(baseUrl(), bridgeToken(), 'highlight', { ids }, exec.signal, { timeoutMs: 5000 }).catch(() => null)
          }
          return { nodeErrors: errors, warnings, highlighted: ids }
        })
    },
  }))

  // M3: expose the active configuration to the agent (read-only convenience).
  // activeView is session-isolated: the client writes per-session values keyed
  // by session id, and this tool reads the value for the CURRENT session.
  // 'canvas' = the user has the ComfyUI split canvas tab selected for this
  // session right now; 'chat' = plain chat tab. The session id comes from the
  // stable tool-run contract (exec.agent.session), falling back to the
  // inherited initiator; when neither is available the caller is told
  // activeViewKnown=false instead of silently assuming chat.
  ctx.tools.register(defineTool({
    name: 'comfyui_config',
    description: 'Show the active ComfyUI connection configuration the canvas plugin is using, ' +
      'plus `activeView` for the CURRENT session ("canvas" when the ComfyUI split canvas tab is ' +
      'selected in the browser for this session, "chat" when the plain chat tab is). ' +
      'Read this before canvas work to confirm the user is on the canvas in THIS session.',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args, exec) {
      const value = readConfig()
      let currentSessionId
      let sessionSource = 'none'
      try {
        currentSessionId = exec?.agent?.session?.id
        if (currentSessionId) sessionSource = 'exec.agent'
      } catch { /* exec contract unavailable */ }
      if (!currentSessionId) {
        try {
          currentSessionId = ctx.get?.('agents')?.currentInitiator?.()?.session?.id
          if (currentSessionId) sessionSource = 'currentInitiator'
        } catch { /* agents service unavailable */ }
      }
      let activeView = 'chat'
      let activeViewKnown = false
      if (currentSessionId && value?.activeViewBySession?.[currentSessionId] === 'canvas') {
        activeView = 'canvas'
        activeViewKnown = true
      } else if (currentSessionId && value?.activeViewBySession?.[currentSessionId] === 'chat') {
        activeView = 'chat'
        activeViewKnown = true
      }
      // P0 handshake: does the BRIDGE actually enforce the token? The DSH side
      // setting alone proves nothing — if the ComfyUI side never got
      // DSH_BRIDGE_TOKEN, _authorized() always passes and the bridge is open.
      // Shared probe (also used by the settings watcher) answers it.
      const token = (value?.bridgeToken || '').trim()
      const probe = await probeBridgeAuth(baseUrl(), token)
      // v0.1.6: bridge install status — lets the agent see at a glance whether
      // the ComfyUI-side bridge node is present and current (vs the version
      // embedded in this plugin package), so it can offer comfyui_setup_bridge.
      const dir = (value?.comfyuiDir || process.env.COMFYUI_DIR || '').trim()
      let bridgeInstalled = false
      let bridgeVersion = null
      let bridgeUpToDate = false
      if (dir) {
        const fs = await import('node:fs')
        const targetRoot = path.join(dir, 'custom_nodes', 'ComfyUI-DSH-Canvas')
        bridgeInstalled = fs.existsSync(path.join(targetRoot, '__init__.py'))
        bridgeVersion = bridgeInstalled ? readVersionOf(targetRoot) : null
        const embeddedRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'comfyui-bridge', 'ComfyUI-DSH-Canvas')
        const embeddedVersion = readVersionOf(embeddedRoot)
        bridgeUpToDate = bridgeInstalled && bridgeVersion !== null && embeddedVersion !== null
          && compareVersions(bridgeVersion, embeddedVersion) >= 0
      }
      return {
        baseUrl: baseUrl(),
        activeView,
        activeViewKnown,
        sessionSource,
        bridgeTokenSet: Boolean(token),
        bridgeAuthEffective: probe.bridgeAuthEffective,
        bridgeAuthProbe: probe.bridgeAuthProbe,
        checkedAt: probe.checkedAt,
        comfyuiDir: value?.comfyuiDir || '',
        launchCommand: value?.launchCommand || '',
        projectDir: resolveProjectDir(),
        bridgeInstalled,
        bridgeVersion,
        bridgeUpToDate,
      }
    },
  }))

  // M4: one-click upgrade — git pull the ComfyUI core, then every git-backed
  // custom node under custom_nodes/. Platform-neutral via the shell service.
  ctx.tools.register(defineTool({
    name: 'comfyui_upgrade',
    description: 'Upgrade the local ComfyUI install: git pull the core, then git pull every git-backed ' +
      'custom node under custom_nodes/. Returns per-repo results. Requires the ComfyUI install directory ' +
      '(set it in Settings → ComfyUI 画布 → ComfyUI 安装目录, or env COMFYUI_DIR).',
    parameters: {
      scope: { type: 'string', enum: ['core', 'nodes', 'all'], description: 'What to upgrade: core only, nodes only, or all (default all).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args) {
      const shell = ctx.shell
      if (!shell || typeof shell.run !== 'function') {
        throw new Error('shell service unavailable — cannot run upgrade commands')
      }
      const value = readConfig()
      const dir = (value?.comfyuiDir || process.env.COMFYUI_DIR || '').trim()
      if (!dir) {
        throw new Error('ComfyUI install directory not set. Set comfyuiDir in Settings → ComfyUI 画布, or env COMFYUI_DIR.')
      }
      const fs = await import('node:fs')
      const path = await import('node:path')
      // WKB review: 69 sequential git pulls can take ~2.3h and hang the agent.
      // Run concurrently (4 lanes), bound the whole batch, and skip dirty
      // checkouts (locally-modified repos are exactly this plugin's own users)
      // instead of hard-pulling into conflicts.
      const CONCURRENCY = 4
      const BATCH_TIMEOUT_MS = 300000 // 5 min total
      const run = async (label, command, workdir) => {
        try {
          const spec = shell.resolve({ command, workdir, timeoutMs: 120000 })
          const result = await shell.run(spec)
          const out = (result.stdout?.text || result.stdout || '').toString().trim()
          const err = (result.stderr?.text || result.stderr || '').toString().trim()
          return { label, exitCode: result.exitCode, ok: result.exitCode === 0, out, err }
        } catch (e) {
          return { label, ok: false, error: e instanceof Error ? e.message : String(e) }
        }
      }
      const results = []
      const scopeArg = args?.scope ?? 'all'
      const jobs = []
      // core: the ComfyUI root is itself a git checkout
      if (scopeArg !== 'nodes' && fs.existsSync(path.join(dir, '.git'))) {
        jobs.push({ label: 'core', workdir: dir })
      }
      // nodes: every git-backed custom node
      if (scopeArg !== 'core') {
        const nodesDir = path.join(dir, 'custom_nodes')
        if (fs.existsSync(nodesDir)) {
          const entries = fs.readdirSync(nodesDir, { withFileTypes: true })
            .filter(d => d.isDirectory())
            .map(d => d.name)
            .sort()
          for (const name of entries) {
            const nodeDir = path.join(nodesDir, name)
            if (fs.existsSync(path.join(nodeDir, '.git'))) {
              jobs.push({ label: `node:${name}`, workdir: nodeDir })
            }
          }
        }
      }
      // dirty probe: empty output = clean (pull safe); non-empty = skip + report
      const skippedDirty = []
      const pullJobs = []
      for (const job of jobs) {
        const probe = await run(job.label, 'git status --porcelain', job.workdir)
        if (probe.ok && probe.out.trim().length > 0) {
          skippedDirty.push({ label: job.label, changedFiles: probe.out.trim().split('\n').length })
        } else {
          pullJobs.push(job)
        }
      }
      // bounded concurrency over the pull queue
      const started = Date.now()
      let cursor = 0
      const worker = async () => {
        while (cursor < pullJobs.length) {
          if (Date.now() - started > BATCH_TIMEOUT_MS) {
            results.push({ label: pullJobs[cursor].label, ok: false, error: 'batch timeout exceeded' })
            cursor++
            continue
          }
          const job = pullJobs[cursor++]
          results.push(await run(job.label, 'git pull', job.workdir))
        }
      }
      await Promise.all(Array.from({ length: Math.min(CONCURRENCY, pullJobs.length) }, worker))
      results.sort((a, b) => a.label.localeCompare(b.label))
      return { upgraded: results.length, skippedDirty, results }
    },
  }))

  // v0.1.6: one-click bridge install — detect / install / update the
  // ComfyUI-side bridge node (comfyui-bridge/ComfyUI-DSH-Canvas) into the
  // local ComfyUI's custom_nodes/. This removes the manual copy step that was
  // the main install-friction point. Idempotent: an already-installed bridge
  // at the same or newer version is left untouched (unless force is given);
  // a stale copy is upgraded. ComfyUI must be restarted to load a new node,
  // which the caller can do here (restart=true) or afterwards via
  // comfyui_reload / the canvas header.
  ctx.tools.register(defineTool({
    name: 'comfyui_setup_bridge',
    description: 'Install or update the ComfyUI-side bridge node (ComfyUI-DSH-Canvas) into the local ComfyUI install. ' +
      'Detects the installed version vs the plugin-embedded one, copies the bridge files into custom_nodes/ (idempotent; ' +
      'a same-or-newer installed version is kept unless force=true), verifies the files landed, and optionally restarts ' +
      'ComfyUI so the new bridge loads. Requires the ComfyUI install directory (Settings → ComfyUI 画布 → ComfyUI 安装目录, ' +
      'or env COMFYUI_DIR). For a cloud ComfyUI, the bridge must be installed on the machine that RUNS ComfyUI — ' +
      'see the README cloud section; this tool only manages a local install.',
    parameters: {
      installDir: { type: 'string', description: 'Optional override for the ComfyUI install directory (default: the configured comfyuiDir or env COMFYUI_DIR).' },
      force: { type: 'boolean', description: 'Reinstall even when the installed version is same or newer (default false).' },
      restart: { type: 'boolean', description: 'Restart ComfyUI after installing so the new bridge loads (default false; uses the configured launchCommand).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args) {
      const fs = await import('node:fs')
      const path = await import('node:path')
      const value = readConfig()
      const dir = (args?.installDir || value?.comfyuiDir || process.env.COMFYUI_DIR || '').trim()
      if (!dir) {
        // Fall back to the launch command's directory when comfyuiDir is empty.
        const cmd = (value?.launchCommand || '').trim()
        if (cmd && (cmd.endsWith('main.py') || cmd.endsWith('main.py"'))) {
          return {
            installed: false,
            error: 'comfyuiDir not set — could not derive the install directory from launchCommand',
            hint: 'Set ComfyUI 安装目录 in Settings → ComfyUI 画布 (e.g. E:\\AI-ComfyUI\\ComfyUI_windows_portable\\ComfyUI) and retry.',
          }
        }
        throw new Error('ComfyUI install directory not set. Set comfyuiDir in Settings → ComfyUI 画布, or env COMFYUI_DIR.')
      }
      const embeddedRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'comfyui-bridge', 'ComfyUI-DSH-Canvas')
      const targetRoot = path.join(dir, 'custom_nodes', 'ComfyUI-DSH-Canvas')
      if (!fs.existsSync(embeddedRoot)) {
        throw new Error(`embedded bridge not found in this plugin package: ${embeddedRoot}`)
      }
      if (!fs.existsSync(path.join(dir, 'custom_nodes'))) {
        return {
          installed: false,
          present: false,
          error: `custom_nodes directory missing: ${path.join(dir, 'custom_nodes')}`,
          hint: 'This does not look like a ComfyUI install root (no custom_nodes/). Double-check the install directory.',
        }
      }
      // ── detect ──
      const readVersion = () => {
        try {
          const vj = path.join(targetRoot, 'version.json')
          if (fs.existsSync(vj)) {
            const parsed = JSON.parse(fs.readFileSync(vj, 'utf8'))
            if (typeof parsed?.version === 'string' && parsed.version) return parsed.version
          }
          const py = path.join(targetRoot, '__init__.py')
          if (fs.existsSync(py)) {
            const m = /__version__\s*=\s*["']([^"']+)["']/.exec(fs.readFileSync(py, 'utf8'))
            if (m) return m[1]
          }
          return null
        } catch {
          return null
        }
      }
      const present = fs.existsSync(path.join(targetRoot, '__init__.py'))
      const installedVersion = present ? readVersion() : null
      const embeddedVersion = readVersionOf(embeddedRoot)
      const upToDate = present && installedVersion !== null && embeddedVersion !== null
        && compareVersions(installedVersion, embeddedVersion) >= 0
      // ── install / update ──
      const needsInstall = !present || !upToDate || args?.force === true
      let copied = []
      if (needsInstall) {
        fs.mkdirSync(path.join(targetRoot, 'entry'), { recursive: true })
        for (const rel of ['__init__.py', 'version.json', 'entry/bridge.js']) {
          const src = path.join(embeddedRoot, rel)
          const dst = path.join(targetRoot, rel)
          if (fs.existsSync(src)) {
            fs.copyFileSync(src, dst)
            copied.push(rel)
          }
        }
      }
      // ── verify ──
      const verified = ['__init__.py', 'entry/bridge.js'].every((rel) => fs.existsSync(path.join(targetRoot, rel)))
      if (!verified) {
        return {
          installed: false,
          present,
          installedVersion,
          embeddedVersion,
          error: 'bridge copy incomplete — expected __init__.py and entry/bridge.js under custom_nodes/ComfyUI-DSH-Canvas',
        }
      }
      // ── optional restart ──
      let restarted = false
      let restartNote = null
      if (args?.restart === true && needsInstall) {
        const cmd = (value?.launchCommand || '').trim()
        const shell = ctx.shell
        if (cmd && shell && typeof shell.start === 'function') {
          try {
            await shell.start({ command: cmd, workdir: dir, cwd: dir })
            restarted = true
          } catch (e) {
            restartNote = e instanceof Error ? e.message : String(e)
          }
        } else {
          restartNote = 'launchCommand not set — restart ComfyUI manually to load the new bridge'
        }
      } else if (needsInstall) {
        restartNote = 'ComfyUI restart needed to load the new bridge (comfyui_reload or the canvas header)'
      }
      return {
        installed: needsInstall,
        present: true,
        installedVersion: needsInstall ? embeddedVersion : installedVersion,
        embeddedVersion,
        upToDate: !needsInstall,
        copied,
        verified: true,
        restarted,
        restartNote,
        targetDir: targetRoot,
      }
    },
  }))

  // v0.1.2: upload ANY local file (image/audio/video/3D) into ComfyUI's input/
  // and optionally point a Load node at it. Uses ComfyUI's NATIVE /upload/image
  // (the input/ directory is ComfyUI's single upload entry regardless of the
  // endpoint name — audio/video/3D land in the same place, consumable by their
  // native Load nodes). Not the bridge: the source file is on the agent's own
  // machine, so the host reads and uploads it. mediaType defaults to a
  // per-extension guess; nodeId + widgetKey let the caller point the matching
  // Load node (LoadImage→image, LoadAudio→audio, LoadVideo→video, 3D→mesh…).
  ctx.tools.register(defineTool({
    name: 'comfyui_attach_file',
    description: 'Upload a local file (image/audio/video/3D/text) into ComfyUI\'s input/ directory and optionally ' +
      'point a Load node at it. Uses ComfyUI\'s native upload endpoint (not the bridge); the source file is read ' +
      'from the agent\'s own machine. mediaType is inferred from the extension when omitted (incl. text/prompts: ' +
      'txt/md/json/csv/yaml/srt). After upload, pass nodeId (and widgetKey for non-image loads) to update the ' +
      'Load node so it takes effect immediately.',
    parameters: {
      path: { type: 'string', required: true, description: 'Absolute local path to the file.' },
      mediaType: { type: 'string', description: 'Optional explicit media type (e.g. video/mp4, audio/wav, model/gltf-binary); defaults to a per-extension guess.' },
      nodeId: { type: 'number', description: 'Optional Load node id to point at the uploaded file.' },
      widgetKey: { type: 'string', description: 'Optional widget name on the Load node (default "image"; use audio/video/mesh for other Loads).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const fs = await import('node:fs')
      const path = await import('node:path')
      const filePath = args.path
      if (!fs.existsSync(filePath)) throw new Error(`file not found: ${filePath}`)
      const buf = fs.readFileSync(filePath)
      const filename = path.basename(filePath)
      const ext = path.extname(filePath).toLowerCase().replace('.', '')
      const mediaType = args.mediaType || mediaTypeOf(ext) || 'application/octet-stream'
      // ComfyUI's /upload/image is the input/ upload entry; it accepts any
      // file type there. overwrite=false dedups a clashing name and returns the
      // actually-stored name.
      const form = new FormData()
      form.append('image', new Blob([buf]), filename)
      form.append('type', 'input')
      form.append('overwrite', 'false')
      const res = await fetch(`${baseUrl()}/upload/image`, {
        method: 'POST',
        body: form,
        signal: exec.signal,
      })
      if (!res.ok) throw new Error(`upload HTTP ${res.status}`)
      const data = await res.json()
      const stored = (data.subfolder ? `${data.subfolder}/${data.name}` : data.name)
      const out = { filename: stored, name: data.name, subfolder: data.subfolder ?? '', type: data.type ?? 'input', mediaType }
      if (args.nodeId != null) {
        const key = args.widgetKey || 'image'
        const r = await sendCommand(baseUrl(), bridgeToken(), 'set_param', { nodeId: args.nodeId, key, value: stored }, exec.signal)
        out.nodeId = args.nodeId
        out.updated = r
      }
      return out
    },
  }))

  // v0.1.1: upload a local image into ComfyUI's input/ and optionally point a
  // LoadImage node at it. Uses ComfyUI's NATIVE /upload/image (not the bridge):
  // the source file is on the agent's own machine, so the host reads and
  // uploads it — same trust model as get_outputs' /history + /view. The bridge
  // token does not apply (native endpoint, not /dsh-bridge/*).
  // DEPRECATED (v0.1.2): superseded by comfyui_attach_file (image is a subset
  // of its media types). Kept for back-compat; removed in a future version.
  ctx.tools.register(defineTool({
    name: 'comfyui_attach_image',
    description: '[DEPRECATED — use comfyui_attach_file instead] Upload a local image into ComfyUI\'s input/ ' +
      'directory and optionally point a LoadImage node at it. Uses ComfyUI\'s native /upload/image (not the bridge), ' +
      'so the source file is read from the agent\'s own machine. After upload, pass nodeId to update a LoadImage ' +
      'node\'s image widget to the new filename so it takes effect immediately.',
    parameters: {
      image: { type: 'string', required: true, description: 'Absolute local path to the image file (png/jpeg/webp/gif).' },
      nodeId: { type: 'number', description: 'Optional LoadImage node id to point at the uploaded file.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      const fs = await import('node:fs')
      const path = await import('node:path')
      const filePath = args.image
      if (!fs.existsSync(filePath)) throw new Error(`image not found: ${filePath}`)
      const buf = fs.readFileSync(filePath)
      const filename = path.basename(filePath)
      // Node 22: global FormData + Blob are available; fetch sends a real
      // multipart body when handed a FormData. overwrite=false lets ComfyUI
      // dedup a clashing name and return the actually-stored name.
      const form = new FormData()
      form.append('image', new Blob([buf]), filename)
      form.append('type', 'input')
      form.append('overwrite', 'false')
      const res = await fetch(`${baseUrl()}/upload/image`, {
        method: 'POST',
        body: form,
        signal: exec.signal,
      })
      if (!res.ok) throw new Error(`upload HTTP ${res.status}`)
      const data = await res.json()
      // ComfyUI returns { name, subfolder, type }; LoadImage's image widget
      // expects "subfolder/name" when a subfolder is present, else "name".
      const stored = (data.subfolder ? `${data.subfolder}/${data.name}` : data.name)
      const out = { filename: stored, name: data.name, subfolder: data.subfolder ?? '', type: data.type ?? 'input' }
      if (args.nodeId != null) {
        const r = await sendCommand(baseUrl(), bridgeToken(), 'set_param', { nodeId: args.nodeId, key: 'image', value: stored }, exec.signal)
        out.nodeId = args.nodeId
        out.updated = r
      }
      return out
    },
  }))

  // v0.1.1: inject conversation text into the canvas. With nodeId: write a
  // widget directly. With newClass: create a source node, set its widget, and
  // optionally connect it to a target input — a one-step "conversation text →
  // canvas node". The bridge's inject_text branch composes add_node + set_param
  // + connect; set_param alone already covers "write an existing widget", so
  // inject_text is the convenience for "create a new source and wire it".
  ctx.tools.register(defineTool({
    name: 'comfyui_inject_text',
    description: 'Inject text into the live ComfyUI canvas. With nodeId: writes the text to that ' +
      'node\'s widget (default key "text"). With newClass: creates a new node of that class, sets its ' +
      'widget, and (if targetId given) connects it to the target input — a text source you can wire. ' +
      'A convenience wrapper over add_node + set_param + connect for one-step conversation text → canvas node.',
    parameters: {
      text: { type: 'string', required: true, description: 'The text to inject.' },
      nodeId: { type: 'number', description: 'Target node id to write the widget directly (mode 1).' },
      widgetKey: { type: 'string', description: 'Widget name to write (default "text").' },
      newClass: { type: 'string', description: 'Create a new node of this class as the text source, e.g. "CLIPTextEncode" (mode 2).' },
      targetId: { type: 'number', description: 'Connect the new node\'s output to this target node input.' },
      targetSlot: { type: 'number', description: 'Target input slot index for the connection.' },
      sourceSlot: { type: 'number', description: 'Source output slot index, default 0.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'inject_text', {
        text: args.text,
        nodeId: args.nodeId,
        widgetKey: args.widgetKey ?? 'text',
        newClass: args.newClass,
        targetId: args.targetId,
        targetSlot: args.targetSlot,
        sourceSlot: args.sourceSlot ?? 0,
      }, exec.signal, { timeoutMs: 10000 })
    },
  }))

  // v0.1.1: export the current canvas as API-format workflow JSON — the format
  // ComfyUI's /prompt and comfy-cli run_workflow consume. Bridges the live
  // canvas to headless/MCP batch runs: build a graph on the canvas, export it,
  // then run it at scale via comfy-cli run_workflow.
  ctx.tools.register(defineTool({
    name: 'comfyui_export_api',
    description: 'Export the current ComfyUI canvas as an API-format workflow JSON (the format ComfyUI\'s ' +
      '/prompt and comfy-cli run_workflow consume). Use to bridge the live canvas to headless/MCP batch ' +
      'runs: build a graph on the canvas, export it, then run it at scale. Run comfyui_debug first if the ' +
      'canvas has unconnected required inputs (graphToPrompt will reject them).',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'export_api', {}, exec.signal, { timeoutMs: 10000 })
    },
  }))

  // v0.1.6: clear the live canvas to a blank workflow — the "new workflow"
  // semantics from the canvas ops SOP. Destructive (the current graph is
  // discarded on the canvas), so the agent should read_workflow first to
  // retain a copy when the user might want it back.
  ctx.tools.register(defineTool({
    name: 'comfyui_clear',
    description: 'Clear the live ComfyUI canvas to a blank workflow (new-workflow start). This discards ' +
      'the current graph from the canvas view; run comfyui_read_workflow first if you need a copy. ' +
      'Use when starting to build a new workflow rather than editing the existing one.',
    parameters: {
      confirm: { type: 'boolean', description: 'Must be true to clear (guards against accidental destructive clears). Default false.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      if (args?.confirm !== true) {
        return {
          cleared: false,
          error: 'confirm: true is required to clear the canvas — this discards the current graph from the view',
        }
      }
      return sendCommand(baseUrl(), bridgeToken(), 'clear', {}, exec.signal, { timeoutMs: 8000 })
    },
  }))

  // v0.1.6: group nodes into named groups — organizing complex workflows into
  // logical blocks (prompt area / sampler area / output area). Delegates to
  // the bridge's LiteGraph group API; `list` also reports whether the running
  // ComfyUI frontend exposes grouping at all, so the agent can degrade
  // gracefully on custom frontends that lack it.
  ctx.tools.register(defineTool({
    name: 'comfyui_group',
    description: 'Organize nodes into named groups on the live ComfyUI canvas (LiteGraph groups). ' +
      'Actions: add (create a group, optionally with nodes), rename, add_nodes, remove_nodes, list. ' +
      'Use for complex workflows so logical blocks (prompt/sampler/output…) stay readable. ' +
      '`list` returns groupingAvailable=false on frontends without LiteGraph groups — degrade gracefully then.',
    parameters: {
      action: { type: 'string', enum: ['add', 'rename', 'add_nodes', 'remove_nodes', 'list'], required: true, description: 'Group operation.' },
      title: { type: 'string', description: 'Group title (required for add; optional for rename).' },
      id: { type: 'number', description: 'Group index (from list) for rename/add_nodes/remove_nodes.' },
      nodeIds: { type: 'array', description: 'Node ids to add to / remove from the group.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args, exec) {
      return sendCommand(baseUrl(), bridgeToken(), 'group', {
        action: args.action,
        title: args.title,
        id: args.id,
        nodeIds: args.nodeIds,
      }, exec.signal, { timeoutMs: 8000 })
    },
  }))

  // v0.1.2 C: node development/debug tools — read / edit custom-node source and
  // reload ComfyUI so the change takes effect, all from the conversation. The
  // canvas doubles as the observation window: edit → reload → run a minimal
  // workflow on the canvas → see the result, without leaving the chat.
  const requireComfyUIDir = async () => {
    const value = readConfig()
    const dir = (value?.comfyuiDir || process.env.COMFYUI_DIR || '').trim()
    if (!dir) throw new Error('ComfyUI install directory not set. Set comfyuiDir in Settings → ComfyUI 画布, or env COMFYUI_DIR.')
    return dir
  }
  const resolveNodeDir = async (root, nodeName) => {
    const { join, resolve, sep } = await import('node:path')
    const nodeDir = join(root, 'custom_nodes', nodeName)
    // Anchor to the custom_nodes directory itself, not the ComfyUI root:
    // a nodeName like "../x" would otherwise resolve inside root but outside
    // custom_nodes and slip past a root-anchored prefix check.
    const nodesReal = resolve(join(root, 'custom_nodes'))
    const nodeReal = resolve(nodeDir)
    if (!nodeReal.startsWith(nodesReal + sep)) throw new Error(`invalid node path: ${nodeName}`)
    return nodeDir
  }
  // Shared helper: list source files under a node dir (top two levels, skipping
  // .git / node_modules / __pycache__ / .venv), or read one file's content.
  const inspectNodeSource = async (nodeDir, relPath) => {
    const fs = await import('node:fs')
    const path = await import('node:path')
    if (relPath) {
      const target = path.resolve(nodeDir, relPath)
      if (!target.startsWith(path.resolve(nodeDir) + path.sep)) throw new Error('path escapes node dir')
      if (!fs.existsSync(target) || !fs.statSync(target).isFile()) throw new Error(`file not found: ${relPath}`)
      return { path: relPath, content: fs.readFileSync(target, 'utf8') }
    }
    const SKIP = new Set(['.git', 'node_modules', '__pycache__', '.venv', 'venv', 'dist', 'build'])
    const tree = []
    const walk = (dir, depth) => {
      if (depth > 2) return
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (SKIP.has(entry.name)) continue
        const full = path.join(dir, entry.name)
        const rel = path.relative(nodeDir, full).replace(/\\/g, '/')
        tree.push({ path: rel, type: entry.isDirectory() ? 'dir' : 'file', size: entry.isFile() ? fs.statSync(full).size : null })
        if (entry.isDirectory()) walk(full, depth + 1)
      }
    }
    walk(nodeDir, 0)
    return { node: path.basename(nodeDir), tree }
  }
  ctx.tools.register(defineTool({
    name: 'comfyui_read_source',
    description: 'Read a ComfyUI custom node\'s source under custom_nodes/<name>/. Without path: list its source tree ' +
      '(top two levels, skipping .git/node_modules/caches). With path: return that file\'s full content. ' +
      'Use during node development to inspect or debug a custom node (including this plugin\'s own bridge node).',
    parameters: {
      node: { type: 'string', required: true, description: 'Custom node directory name under ComfyUI custom_nodes/ (e.g. ComfyUI-DSH-Canvas).' },
      path: { type: 'string', description: 'Optional relative file path inside the node dir to read (omit to list the tree).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args) {
      const fs = await import('node:fs')
      const dir = await requireComfyUIDir()
      const nodeDir = await resolveNodeDir(dir, args.node)
      if (!fs.existsSync(nodeDir)) throw new Error(`custom node not found: ${args.node}`)
      return inspectNodeSource(nodeDir, args.path)
    },
  }))
  ctx.tools.register(defineTool({
    name: 'comfyui_edit_source',
    description: 'Edit a file inside a ComfyUI custom node\'s source (custom_nodes/<name>/). Writes new content to the ' +
      'given relative path; path is confined to the node dir. Pair with comfyui_reload to apply the change: ' +
      'edit → reload → run a minimal workflow on the canvas to verify. This is how you develop/debug a custom node ' +
      'from the conversation without touching the ComfyUI UI.',
    parameters: {
      node: { type: 'string', required: true, description: 'Custom node directory name under ComfyUI custom_nodes/.' },
      path: { type: 'string', required: true, description: 'Relative file path inside the node dir to write (e.g. __init__.py or entry/bridge.js).' },
      content: { type: 'string', required: true, description: 'Full new file content (replaces the file).' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(args) {
      const fs = await import('node:fs')
      const path = await import('node:path')
      const dir = await requireComfyUIDir()
      const nodeDir = await resolveNodeDir(dir, args.node)
      const target = path.resolve(nodeDir, args.path)
      if (!target.startsWith(path.resolve(nodeDir) + path.sep)) throw new Error('path escapes node dir')
      fs.mkdirSync(path.dirname(target), { recursive: true })
      fs.writeFileSync(target, args.content, 'utf8')
      return { node: args.node, path: args.path, bytes: Buffer.byteLength(args.content, 'utf8'), ok: true }
    },
  }))
  ctx.tools.register(defineTool({
    name: 'comfyui_reload',
    description: 'Restart the local ComfyUI so custom-node source changes take effect: find the process listening on ' +
      'the ComfyUI port, kill it, then relaunch via the configured launch command. Use after comfyui_edit_source, ' +
      'then run a minimal workflow on the canvas to verify the change. Warns about running generations; cancels ' +
      'pending work by restarting.',
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: true }, render: renderJSON },
    async execute(_args) {
      const value = readConfig()
      const base = baseUrl()
      const port = (() => { try { return new URL(base).port || '8188' } catch { return '8188' } })()
      const cmd = (value?.launchCommand || '').trim()
      const workdir = (value?.comfyuiDir || process.env.COMFYUI_DIR || '').trim() || undefined
      const shell = ctx.shell
      if (!shell || typeof shell.run !== 'function') throw new Error('shell service unavailable — cannot reload ComfyUI')
      if (!cmd) throw new Error('launchCommand is empty — set it in Settings → ComfyUI 画布 to allow reload')
      // netstat -ano / findstr / taskkill are Windows-specific; on other
      // platforms the PID hunt and kill would silently no-op and the tool
      // would report a false success. Fail loud instead of pretending.
      if (process.platform !== 'win32') {
        throw new Error('comfyui_reload currently supports Windows only (netstat/taskkill); on macOS/Linux restart ComfyUI manually')
      }
      // 1. find the PID on the ComfyUI port
      const netstat = await shell.run(shell.resolve({ command: `netstat -ano | findstr :${port} | findstr LISTENING`, timeoutMs: 15000 }))
      const out = (netstat.stdout?.text || netstat.stdout || '').toString()
      const pids = [...new Set([...out.matchAll(/\s(\d+)\s*$/gm)].map(m => m[1]))]
      // 2. kill those PIDs (best-effort; ignore failures so relaunch still runs).
      //    taskkill is a host-level operation outside the session workspace, so
      //    run it unconfined like the launch below — the default workspace-write
      //    sandbox would deny it and the old process would survive, blocking the
      //    port and making the reload look like a no-op.
      const killed = []
      for (const pid of pids) {
        try {
          await shell.run(shell.resolve({
            command: `taskkill /PID ${pid} /F`,
            timeoutMs: 10000,
            sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
          }))
          killed.push(pid)
        } catch { /* already gone or denied — proceed */ }
      }
      // 3. relaunch via the same unconfined path the Start button uses
      shell.start(shell.resolve({
        command: cmd,
        ...(workdir ? { workdir } : {}),
        sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: process.cwd() },
      }))
      // 4. wait for /system_stats to come back
      const deadline = Date.now() + 45000
      let up = false
      let lastErr = ''
      while (Date.now() < deadline) {
        try {
          const res = await fetch(`${base}/system_stats`, { signal: AbortSignal.timeout(3000) })
          if (res.ok) { up = true; break }
        } catch (e) { lastErr = e instanceof Error ? e.message : String(e) }
        await new Promise((done) => setTimeout(done, 1500))
      }
      return { reloaded: up, killed, port, launched: cmd, error: up ? null : (lastErr || 'timed out waiting for ComfyUI to come back') }
    },
  }))

  // v0.1.2: register the skills (SOPs) that teach the agent HOW to use the
  // canvas/admin tools in the right order. Runtime skills ship inside the
  // plugin, so `dsh plugin add` installs them with zero extra steps.
  const skills = ctx.skills ?? ctx.get?.('skills')
  if (skills && typeof skills.register === 'function') {
    skills.register({
      name: 'comfyui-canvas-ops',
      source: 'runtime',
      description: 'Operate the live ComfyUI canvas with the agent: read/edit/run workflows, fetch outputs.',
      whenToUse: 'Use when the user asks to make an image, edit a workflow, adjust a node, run the canvas, or fetch its outputs.',
      content: [
        '# ComfyUI 画布操作 SOP',
        '',
        '## 操作模式总则（先判断场景再动手）',
        '',
        '| 场景 | 做法 | 禁止 |',
        '|---|---|---|',
        '| 修改现有工作流 | 当前画布直接增量编辑（set_param/connect/add_node/remove_node/inject_text） | 不要 load_workflow 整图替换（冲掉未保存现场） |',
        '| 新建工作流 | 先 comfyui_clear 开空白画布再搭 | 不要在有内容/运行记录的画布上直接覆盖 |',
        '| 搭建复杂工作流 | 用 comfyui_group 组节点分组（提示词区/采样区/输出区…） | 不要所有节点平铺一团 |',
        '| 快速验证/批量/无人值守 | 走 API/MCP 路径（mcp-comfyui 或原生 /prompt 直接提交），不必可视化 | 不要为展示而无谓地在画布上一步步搭 |',
        '',
        '双路径选择：可视调优/用户在看画布 → 画布路径（bridge 命令）；快速验证/批量/用户未开画布 → API/MCP 路径（导出 workflow 直交，或 mcp-comfyui 工具）。',
        '',
        '当用户要求用画布作图 / 修改工作流 / 出图时，按此顺序执行：',
        '',
        '1. comfyui_config —— 确认画布在前台（activeView=canvas）、连接正常、bridgeInstalled；不在画布视图时先提示用户切到 ComfyUI 标签',
        '2. comfyui_read_workflow —— 读取当前图，向用户复述关键节点并确认要改什么',
        '3. 按需修改：comfyui_set_param（改参数）、comfyui_connect（连线）、comfyui_add_node / comfyui_remove_node（增删节点）、comfyui_inject_text（注入文本）',
        '4. 修改前说明改了哪些；运行前用 comfyui_debug 校验（不触发执行）',
        '5. comfyui_run 运行（可传 overrides 临时覆盖参数，跑完还原画布）',
        '6. comfyui_get_outputs 取回出图（支持 images/videos/gifs/audio）',
        '7. 用 vision 工具自检出图质量 → 不达标回 step 3 迭代；达标再交付',
        '',
        '安全约定：读取永远只读；写操作（改节点/运行/替换图/清空画布）先向用户确认；批量用 comfyui_batch_run；清空画布（comfyui_clear）前先 comfyui_read_workflow 留存现场。',
      ].join('\n'),
    })
    skills.register({
      name: 'comfyui-admin-ops',
      source: 'runtime',
      description: 'Manage the ComfyUI environment: configure, launch, upgrade, and list/install custom nodes.',
      whenToUse: 'Use when the user asks to configure ComfyUI, start it, update it, upgrade its nodes, install the bridge node, or manage installed custom nodes.',
      content: [
        '# ComfyUI 环境管理 SOP',
        '',
        '当用户要求配置 / 启动 / 更新 ComfyUI、管理已装节点时，按此顺序执行：',
        '',
        '0. 桥接节点一键安装 —— 先 comfyui_config 看 bridgeInstalled / bridgeUpToDate：',
        '   - 未装或过期 → comfyui_setup_bridge（自动复制桥接节点进 custom_nodes/；baseUrl 指向云端时按 README 云端小节在云机操作）',
        '   - 已装且最新 → 跳过；说明安装目录是本机路径，云端场景的 installDir 无效',
        '1. 配置管理 —— comfyui_config 读/改连接（baseUrl / port / networkMode / 桥接 token / 启动命令 / rail 宽度）',
        '2. 启动 / 状态 —— 用设置里的启动命令拉起 ComfyUI（launchRequested 机制），轮询 /system_stats 确认在线；读 launchError 排查启动失败',
        '3. 更新 —— comfyui_upgrade：git pull 核心 + 全部节点（scope=core/nodes/all），失败不中断其余',
        '4. 节点管理 —— 用文件系统工具列出 custom_nodes/、识别 git 托管与手工目录；安装/更新节点用 shell 走 git 或 pnpm',
        '',
        '安全约定：更新 / 重启 / 装节点是重操作 —— 先确认、讲清影响（如「更新 60+ 节点」「重启中断生成」）；更新前检查是否有运行中任务。',
      ].join('\n'),
    })
    skills.register({
      name: 'comfyui-video-audio-ops',
      source: 'runtime',
      description: 'Create video with audio/voiceover on ComfyUI: generate video, attach audio, fetch the merged file.',
      whenToUse: 'Use when the user wants a video with sound / voiceover / music track produced through the ComfyUI canvas.',
      content: [
        '# ComfyUI 音画创作 SOP',
        '',
        '当用户要求「视频 + 配音 / 音轨 / 音乐」时，按此顺序执行：',
        '',
        '1. 生成视频 —— 读画布上的视频工作流（如 Wan / Kling / LTX），comfyui_run 运行',
        '2. 生成音频 —— 用 comfyui_attach_file 把 TTS 生成的 wav / 音轨文件上传进 ComfyUI input/（供 LoadAudio 节点）',
        '3. 音画合成 —— 走 VideoAddAudio 或对应音画工作流，comfyui_run 合并',
        '4. comfyui_get_outputs 取回含音轨的视频文件（URL/路径）',
        '5. 交付时给出文件路径/URL；DSH 不播放，用户本地打开',
        '',
        '提示：TTS 音频只是「传输的一类文件」，上传即用；播放能力由 DSH 生态负责，本插件只做传输。',
      ].join('\n'),
    })
    skills.register({
      name: 'comfyui-dev-ops',
      source: 'runtime',
      description: 'Develop or debug a ComfyUI custom node from the conversation: read its source, edit it, reload ComfyUI, and verify on the canvas.',
      whenToUse: 'Use when the user wants to develop, modify, or debug a ComfyUI custom node (including this plugin\'s bridge node) — read/edit its source, restart ComfyUI, or verify a change on the canvas.',
      content: [
        '# ComfyUI 节点开发调试 SOP',
        '',
        '当用户要求开发 / 修改 / 调试 ComfyUI 自定义节点（含本插件的桥接节点）时，按此循环执行：',
        '',
        '1. comfyui_read_source —— 读目标节点源码（列出目录树或读单个文件，如 __init__.py / entry/bridge.js），先弄清结构与注册方式',
        '2. comfyui_edit_source —— 修改源码（受控写文件，仅限该节点目录内）',
        '3. comfyui_reload —— 重启 ComfyUI 使改动生效（会中断运行中任务，先确认）；等 /system_stats 恢复在线',
        '4. 画布验证 —— 在画布上搭一个用到该节点的最小工作流，comfyui_run 运行',
        '5. comfyui_get_outputs 看结果 → 不达标回 step 2 迭代',
        '',
        '安全约定：改的是 custom_nodes/ 下的本机源码 —— 写操作先确认、讲清改哪个文件；重启前检查是否有运行中任务；改坏了节点可能让 ComfyUI 起不来（保留备份 / 可回退）。',
      ].join('\n'),
    })
  }
}
